...
NATIVE_BASENAME="detector"
NATIVE_DIR="${NATIVE_DIR:-services}"
NATIVE_SPECIAL_FILE="${NATIVE_SPECIAL_FILE:-/$NATIVE_DIR/$NATIVE_BASENAME.special}"
NATIVE_SOCKET="${NATIVE_SOCKET:-/$NATIVE_DIR/$NATIVE_BASENAME.sock}"
NATIVE_PATH_LOCAL="${NATIVE_PATH_LOCAL:-./$NATIVE_BASENAME}"
NATIVE_PATH_REMOTE="${NATIVE_PATH_REMOTE:-/video_input/$NATIVE_BASENAME}"
...
YOLOV3_CFG_BASENAME="yolov3-tiny.cfg"
...
YOLOV3_WEIGHTS_BASENAME="yolov3-tiny.weights"
...
killall -9 proxy_attestation_server $BACKEND-veracruz-server veracruz-client $BACKEND-runtime-manager
...
$POLICY_GENERATOR_PATH \
    ...
    --capability "/$PROGRAM_DIR/:w,/$NATIVE_DIR/:w,/video_input/:w" \
    ...
    --capability "/$PROGRAM_DIR/:r,/$PROGRAM_DATA_DIR/:r,/$VIDEO_INPUT_DIR/:rwx,/program_internal/:rw,/$OUTPUT_DIR/:w,/services/:rw,stdout:w,stderr:w" \
    --native-module-entry-point "/tmp/nmm/$NATIVE_BASENAME/$VIDEO_INPUT_DIR/$NATIVE_BASENAME" \
    --native-module-name="$NATIVE_BASENAME" \
    --native-module-special-file "$NATIVE_SPECIAL_FILE" \
    --native-module-socket-path "$NATIVE_SOCKET" \
...
if [ -z $SERVERLESS ]; then 
    ...
    RUST_LOG=info RUNTIME_ENCLAVE_BINARY_PATH=$RUNTIME_MANAGER_PATH $SERVER_PATH $POLICY_PATH &
fi
...
echo $NATIVE_PATH_REMOTE=$NATIVE_PATH_LOCAL

echo "=============Provisioning native"
RUST_LOG=error $CLIENT_PATH $POLICY_PATH \
    --program $NATIVE_PATH_REMOTE=$NATIVE_PATH_LOCAL \
    --identity $PROGRAM_CLIENT_CERT_PATH \
    --key $PROGRAM_CLIENT_KEY_PATH || exit
