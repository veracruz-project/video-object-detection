...
def make_labels(s):
    ...
    for word in l:
        if word == ' ':
            os.system('convert -fill black -background white -bordercolor white -pointsize %d label:"\ " 32_%d.png'%(s,s/12-1))
        if word == '@':
            os.system('convert -fill black -background white -bordercolor white -pointsize %d label:"\@" 64_%d.png'%(s,s/12-1))
        elif word == '\\':
            os.system('convert -fill black -background white -bordercolor white -pointsize %d label:"\\\\\\\\" 92_%d.png'%(s,s/12-1))
        elif ord(word) in [9,10,11,12,13,14]:
            pass
        else:
            os.system("convert -fill black -background white -bordercolor white -pointsize %d label:%s \"%d_%d.png\""%(s,pipes.quote(word), ord(word),s/12-1))
